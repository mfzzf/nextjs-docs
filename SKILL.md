---
name: nextjs-docs
description: 'Offline mirror of **Next.js 16.2.6 docs** (`nextjs.org/docs/llms.txt` + nested `pages/llms.txt` → 406 raw markdown pages, ~4.4 MB). Covers App Router and Pages Router — Getting Started, every Guide (auth, forms, MDX, testing, OpenTelemetry, PWA, migrations, multi-zone, view transitions), full API Reference (directives `use cache` / `use cache: private` / `use cache: remote` / `use client` / `use server`, components, every file convention, every function, every `next.config.js` option, CLI, Adapters, Edge Runtime, Turbopack), Architecture, Community. Use when you need an authoritative Next.js detail — config option, hook, file convention, App↔Pages migration, RSC/Server Action semantics, caching/revalidation model. Trigger on `next.config.js`, `app/`, `pages/`, `use server`/`use client`/`use cache`, `revalidateTag`, `generateMetadata`, `next/image`, `next/link`, `next/font`, `create-next-app`, Server Actions, RSC, ISR, PPR, Turbopack, "App Router", "Pages Router", "Next.js 文档".'
---
